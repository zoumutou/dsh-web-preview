...
// ~/... 展开为用户主目录（如 /Users/zoumutou/...），让对话里的 ~ 路径可直接定位
...
export function apply(ctx) {
  ...
  const render404 = (res, rel, root, sid, autoHits, findQuery, findResults) => {
    const hitsHtml = findQuery
      ...
      : (autoHits && autoHits.length
          ? '<p class="sec">同名文件（' + autoHits.length + ' 个，请选择打开目标）：</p>' + autoHits.map((m) =>
              '<a class="hit" href="' + relHref(sid, m.rel) + '">📄 ' + escHtml(m.rel) + '</a>').join('')
          : '')
    const html = '<!doctype html><html><head><meta charset="utf-8"><title>文件不存在</title><style>' +
      ...
      'html[data-theme="light"] .card{background:#f6f8fa;border:1px solid #d0d7de;box-shadow:0 12px 40px rgba(0,0,0,.12)}' +
      'html[data-theme="light"] code{background:#eaeef2;color:#116329}' +
      'html[data-theme="light"] p{color:#57606a}' +
      'html[data-theme="light"] .root{color:#8c959f}' +
      'html[data-theme="light"] .find input{background:#ffffff;color:#1f2328;border:1px solid #d0d7de}' +
      'html[data-theme="light"] .hit{background:#f6f8fa;border:1px solid #d0d7de;color:#116329}' +
      'html[data-theme="light"] .none,.err{color:#57606a}' +
  ...
  const codePage = (name, content) => {
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><title>' + escHtml(name) + '</title><style>' +
      ...
      'html[data-theme="light"]{color-scheme:light}html[data-theme="dark"]{color-scheme:dark}' +
      'html[data-theme="light"] body{background:#ffffff;color:#1f2328}' +
      'html[data-theme="light"] .head{background:rgba(255,255,255,.92);border-bottom-color:#d0d7de;color:#57606a}' +
      'html[data-theme="light"] .ln{color:#8c959f}' +
  ...
  const mdToHtml = (name, src) => {
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><title>' + escHtml(name) + '</title><style>' +
      ...
      'html[data-theme="light"] body{background:#ffffff;color:#1f2328}' +
      'html[data-theme="light"] .head{background:rgba(255,255,255,.92);border-bottom-color:#d0d7de;color:#57606a}' +
      'html[data-theme="light"] .md h1{border-bottom-color:#d0d7de}' +
      'html[data-theme="light"] .md code{background:#f2f4f7;border:1px solid #d0d7de}' +
      'html[data-theme="light"] .md pre.code{background:#f6f8fa;border:1px solid #d0d7de}' +
      'html[data-theme="light"] .md blockquote{border-left-color:#d0d7de;color:#57606a}' +
      'html[data-theme="light"] .md a{color:#0969da}' +
  ...
  async function serveStatic(req, res, segs, sess, sid, findQuery) {
    ...
    // __abs__ 路由：直接托管根目录外的本地文件（对话里点击 ~/... 或 /Users/... 绝对路径）
    const absRoute = segs[0] === '__abs__'
    const abs = absRoute
      ? '/' + segs.slice(1).map((s) => { try { return decodeURIComponent(s) } catch (e) { return s } }).join('/')
      : (segs.length ? root + '/' + segs.join('/') : root)
    ...
    const notFound = async () => {
      ...
      if (!findQuery) {
        ...
        if (name) {
          ...
          const found = await findFiles(root, q, 40)
          hits = found.filter((m) => String(m.name).toLowerCase() === String(name).toLowerCase()).slice(0, 12)
        }
      }
    ...
    if (!absRoute && !ctx.fs.contains(rootT, target)) {
      await notFound()
      return
    }
  ...
  // ~/... 展开为用户主目录（如 /Users/zoumutou/...），让对话里的 ~ 路径可直接定位磁盘
  const expandTilde = (p) => {
    if (p === '~') return homedir()
  ...
  api['resolve-path'] = async (args) => {
    const sess = getSession(args && args.sid)
    const path0 = args && typeof args.path === 'string' ? args.path.trim() : ''
    if (!sess.root || !path0) return { ok: false, error: '路径无效' }
    const path = expandTilde(path0)
    ...
    // 2) 相对路径（如 docs/README.md）在根目录内精确存在 → 直接打开（`~` 已在上面展开）
    if (name && path.indexOf('/') >= 0 && !path.startsWith('/')) {
      const rel2 = path.replace(/^\.?\//, '')
      const e = await tryStat(sess.root + '/' + rel2)
      if (e && e.info.type === 'file') return { ok: true, url: baseUrl + '/' + rel2 }
    }
    // 3) 绝对路径在磁盘上真实存在但不在预览根目录内 → 经 __abs__ 路由直接打开（精确目标，不猜同名）
    if (name && path.startsWith('/')) {
      const disk = await tryStat(path)
      if (disk && disk.info.type === 'file') {
        return { ok: true, url: baseUrl + '/__abs__' + path, note: '已直接打开磁盘文件（工作区外）' }
      }
    }
    // 4) 工作区内按同名搜索：唯一命中直接打开；多个命中返回候选列表，由用户选择
    if (name) {
      const dot = name.lastIndexOf('.')
      const q = dot > 0 ? name.slice(0, dot) : name
      const hits = await findFiles(sess.root, q, 50)
      const pool = hits
        .filter((h) => String(h.name).toLowerCase() === String(name).toLowerCase())
        .map((h) => ({ name: h.name, rel: h.rel }))
      if (pool.length === 1) {
        return { ok: true, url: baseUrl + '/' + pool[0].rel, note: '已打开工作区中的同名文件' }
      }
      if (pool.length > 1) {
        return {
          ok: false,
          error: '工作区中找到 ' + pool.length + ' 个同名文件，请选择要打开的目标',
          candidates: pool,
          file: name
        }
      }
  ...
  // 链接有效性校验：绝对路径看磁盘是否存在；相对路径只在预览根目录内精确查找（不做同名回退）
  api['lookup-file'] = async (args) => {
    const sess = getSession(args && args.sid)
    const p0 = args && typeof args.path === 'string' ? args.path.trim() : ''
    if (!sess.root || !p0) return { ok: false, error: '路径无效' }
    const p = expandTilde(p0)
    if (p.startsWith('/')) {
      const e = await tryStat(p)
      return { ok: true, found: !!(e && e.info.type === 'file') }
    }
    const rel = p.replace(/^\.?\//, '')
    const e = await tryStat(sess.root + '/' + rel)
    return { ok: true, found: !!(e && e.info.type === 'file') }
